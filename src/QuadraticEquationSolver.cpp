...
QuadraticEquationSolver::Solution QuadraticEquationSolver::solve(const Data& stringCoeff) const {
    ...
    if (a == 0) {
        if (b == 0) {
            ...
            // single root, no xmin
            return Solution(stringCoeff, root, true);
        }
    }
    ...
    if (discriminant < 0) {
        ...
        return Solution(stringCoeff, -sum / 2);
    }
    ...
    return Solution(stringCoeff, root1, root2, xmin);
}