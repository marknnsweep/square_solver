...
public:
    ...
    struct Solution {
        ...
        Solution(Data coeff, double _xmin) : coefficients(coeff), xmin(_xmin), no_roots(true) {}
        Solution(Data coeff, double root, bool) : coefficients(coeff), root1(root), no_xmin(true) {}
        Solution(Data coeff, double _xmin, double root) : coefficients(coeff), xmin(_xmin), root1(root) {}
        Solution(Data coeff, double _xmin, double _root1, double _root2) : coefficients(coeff), xmin(_xmin), root1(_root1), root2(_root2) {}
    ...
    friend std::ostream& operator<<(std::ostream& os, const Solution& solution) {
        ...
        if (solution.incorrect_input) {
            ...
            os << "(";
            os << (solution.root1.has_value() ? std::to_string(solution.root1.value()) : "No root 1");
            os << ", ";
            os << (solution.root2.has_value() ? std::to_string(solution.root2.value()) : "No root 2");
            os << (solution.xmin.has_value() ? ") Xmin=" + std::to_string(solution.xmin.value()) : ") No Xmin");
        }
