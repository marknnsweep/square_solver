...
template <typename Collection, size_t Size,
...
public:
  ...
  public:
    explicit IteratorWrapper(Iterator start, Iterator finish)
        : current(start), end(finish) {
      advance();
    }

    const TupleType &operator*() const { return current_tuple; }
  ...
  private:
    ...
    static TupleType
    to_tuple(const std::vector<std::optional<ValueType>> &elements) {
      return create_tuple(elements, std::make_index_sequence<Size>{});
    }

    template <std::size_t... Indexes>
    static TupleType
    create_tuple(const std::vector<std::optional<ValueType>> &elements,
                 std::index_sequence<Indexes...>) {
      // use some template magic to unpack values into tuple
      return std::make_tuple(
          (Indexes < elements.size() ? elements[Indexes] : std::nullopt)...);
    }
  };
